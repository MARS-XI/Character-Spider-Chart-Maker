...
# my modules
# import script to create DataFrame of the character in a console session
import character_dataframe as cd
...
def make_spider(df, col, col_num, title, color):
    """
    plot character stats into a spide chart (support func to fig_setup)
    INPUT (passed by fig_setup func):
    df = dataframe, col = column label, col_num = numeric column index, title = graph's title, color = graph's color
    DATAFRAME: from dict {character_name:[char_stats_list]}
    ...
    # len(index) sets number of xticks (number of statistics for each character)
    ...
    df_columns = [float(n) for n in df[col].tolist()]  # list containing current column (column = character stat list)
...
def fig_setup(df):
    ...
    INPUT:
    - dataframe({char_name:[char_stats_list]}
    OUTPUT:
    - matplotlib plot
    """
    my_dpi = 175
    ...
    for col in df.columns.tolist():  # df.columns is numpy array --> numpy method .tolist()
        make_spider(df, col, col_num, title=f"{col}'s stats", color=colors[col_num])
        col_num += 1
...
if __name__ == '__main__':
    character_stats = cd.make_dataframe()
    fig_setup(character_stats)  # creates spider charts
